...
const { closeSync, copyFileSync, cpSync, mkdirSync, openSync, rmSync, statSync, symlinkSync, writeSync } = require("fs");
...
const ILLEGAL = /[\\<>:"|?*\u0000-\u001f]|[ .]$/;
const SOFT = new Set(["EPERM", "EACCES", "ENOSYS", "ENOTSUP", "EOPNOTSUPP", "UNKNOWN"]);
...
function unsafe(name) {
    for (const part of name.split("/")) {
        if (ILLEGAL.test(part) || RESERVED.test(part)) return true;
    }
    return false;
}

function relink(entry, source) {
    try {
        symlinkSync(entry.link, entry.target);
        return;
    } catch (error) {
        if (!SOFT.has(error.code)) throw error;
    }
    const stats = statSync(source, { throwIfNoEntry: false });
    if (!stats) return;
    if (!stats.isDirectory()) {
        copyFileSync(source, entry.target);
        return;
    }
    if (WINDOWS) {
        try {
            symlinkSync(entry.link, entry.target, "junction");
            return;
        } catch (error) {
            if (!SOFT.has(error.code)) throw error;
        }
    }
    cpSync(source, entry.target, { recursive: true, force: true });
}
    ...
    finish() {
        ...
        for (const entry of this.links) {
            ...
            try {
                ...
                rmSync(entry.target, { recursive: true, force: true, maxRetries: 5 });
                if (entry.type === "2") relink(entry, source);
